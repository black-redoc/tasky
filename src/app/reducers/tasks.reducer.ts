...
export type ActionState = {
  type: string;
  payload: { task: TaskType, tasks?: TaskStateType, project_id?: number | string };
}
  ...
  if (action.type === 'UPDATE_LAST_TASK_CREATED') {
    ...
    if (payload.status !== payload.oldStatus && Boolean(payload.oldStatus)) {
      return {
        ...
        tasks: {
          ...structuredClone(state.tasks),
          [payload.status]: [...state.tasks[payload.status], { ...payload }],
          [payload.oldStatus ?? '']: state.tasks[payload.oldStatus ?? ''].filter((task) => task.id !== payload.id)
        }
      }
    }
    return {
    ...
    return {
      ...
      tasks: Object.entries(state.tasks)
        .filter(([key, _]) => key === action.payload.task.id)
...
export function taskFormReducer(state: typeof taskFormInitialState, action: TaskFormActionState) {
